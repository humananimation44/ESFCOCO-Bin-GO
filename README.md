...
We also utilized Perplexity as a learning and coding assistant for brainstorming, explaining Mediapip/OpenCV concepts, and troubleshooting; we did not train the underlying hand model. Other than OpenCV code, we used Perplexity simply to troubleshoot in extreme matters, with minimal use, and Copilot in VS Code Studio for debugging, as well as open-source HTML (very minimal use, however) for many 

OpenCV Code explanation-
Adopting the camera-tracking model from MediaPipe, we add visuals using simple borders and added functionality to the fingers and added tracking with if, elif and else to have a functional bin.
